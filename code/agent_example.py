...
def agent_calling():
     ...
     print(f"6. Invoke the simple chain by asking a question.\n")     
     response = simple_chain.invoke({"question": "Which city is hotter today: LA or NY?"})

     print(f"7. Inspect the response.\n{response}\n") 
     
     tools = [weather_service]
     print(f"8. Define tools\n{tools}\n")

     agent_system_prompt = load_agent_system_prompt()
     print(f"9. Define agent system prompt\n{agent_system_prompt}\n")

     human_prompt = load_human_prompt()
     print(f"10. Define human prompt\n{human_prompt}\n")
     ...
     print(f"11. Create an agent chat prompt from a ChatPromptTemplate\n{agent_chat_prompt}\n")

     agent_chat_incl_tools_prompt = agent_chat_prompt.partial(
        tools=render_text_description_and_args(list(tools)),
        tool_names=", ".join([t.name for t in tools]),
     )
     print(f"12. Extent the Chat Prompt Template with the given tool names as variables\n{agent_chat_incl_tools_prompt}\n")
     
     memory = ConversationBufferMemory()
     print(f"13. Setup of a ConversationBufferMemory\n{memory}\n")
     ...
     print(f"16.a Response of the agent executor\n{response}\n")
     ...
     print(f"16.b Response of the agent executor\n{response}\n")
     data = { "question": question, "answer":  response['output'], "history": response['history'] }          
     results.append(data)
     ...
     dict = {"input": question}
     response = agent_executor.invoke(dict)
     print(f"16.c Response of the agent executor\n{response}\n")
     data = { "question": question, "answer":  response['output'], "history": response['history'] }          
     results.append(data)
     ...
     dict = {"input": question}
     response = agent_executor.invoke(dict)
     print(f"16.d Response of the agent executor\n{response}\n")
     data = { "question": question, "answer":  response['output'], "history": response['history'] }          
     results.append(data)
